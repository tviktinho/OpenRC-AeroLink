 ...
 */
 ...
 void savePidsToNVS() {
   ...
   preferences.end();
   StaticJsonDocument<64> res; res["status"]="ok"; res["msg"]="PIDs saved"; sendJsonSerial(res);
 }
 ...
 void saveRcCalibrationToNVS() {
    ...
    char key[16];
    for (int i=0;i<8;i++){
      snprintf(key, sizeof(key), "rcmin%d", i); preferences.putInt(key, g_rc_min[i]);
      snprintf(key, sizeof(key), "rcmax%d", i); preferences.putInt(key, g_rc_max[i]);
      snprintf(key, sizeof(key), "rccenter%d", i); preferences.putInt(key, g_rc_center[i]);
      snprintf(key, sizeof(key), "rctrim%d", i); preferences.putInt(key, g_rc_trim[i]);
    }
    preferences.putInt("servo_trim_l", g_servo_trim_l);
    preferences.putInt("servo_trim_r", g_servo_trim_r);
    preferences.end();
    StaticJsonDocument<64> res; res["status"]="ok"; res["msg"]="RC Cal Saved"; sendJsonSerial(res);
 }
 ...
 void loadRcCalibrationFromNVS(){
    ...
    char key[16];
    for (int i=0;i<8;i++){
      snprintf(key, sizeof(key), "rcmin%d", i); g_rc_min[i] = preferences.getInt(key, PWM_MAX);
      snprintf(key, sizeof(key), "rcmax%d", i); g_rc_max[i] = preferences.getInt(key, PWM_MIN);
      snprintf(key, sizeof(key), "rccenter%d", i); g_rc_center[i] = preferences.getInt(key, 1500);
      snprintf(key, sizeof(key), "rctrim%d", i); g_rc_trim[i] = preferences.getInt(key, 0);
    }
 ...
 void performGyroCalibration() {
   ...
   StaticJsonDocument<64> info; info["status"]="info"; info["msg"]="Calibrating Gyro..."; sendJsonSerial(info);
   ...
   StaticJsonDocument<64> res; res["status"]="ok"; res["msg"]="Gyro Calibrated"; sendJsonSerial(res);
 }
 ...
 void performRcCalibration(uint32_t duration_ms) {
    ...
    for (int i=0;i<8;i++){ g_rc_min[i] = 2000; g_rc_max[i] = 1000; } // Invertido intencionalmente para expandir
    
    StaticJsonDocument<64> info; info["status"]="info"; info["msg"]="RC Cal Start"; sendJsonSerial(info);
    
    uint32_t start = millis();
    while (millis() < start + duration_ms) {
      for (int i=0;i<8;i++){
        int us = constrain(map(pkt.p[i], 0, 255, PWM_MIN, PWM_MAX), PWM_MIN, PWM_MAX);
        if (us < g_rc_min[i]) g_rc_min[i] = us;
        if (us > g_rc_max[i]) g_rc_max[i] = us;
      }
      delay(20);
    }
    for (int i=0;i<8;i++) g_rc_center[i] = (g_rc_min[i] + g_rc_max[i]) / 2;
    saveRcCalibrationToNVS();
    g_rc_calibrating = false;
    if (!was_in_failsafe) g_failsafe_active = false;
    StaticJsonDocument<64> res; res["status"]="ok"; res["msg"]="RC Cal Done"; sendJsonSerial(res);
 }
 
 void performCenterServos(uint32_t hold_ms){
    StaticJsonDocument<64> info; info["status"]="info"; info["msg"]="Centering..."; sendJsonSerial(info);
    ...
    delay(hold_ms);
    StaticJsonDocument<64> res; res["status"]="ok"; res["msg"]="Done Centering"; sendJsonSerial(res);
 }
 
 void performCenterAndSave(){
    bool hasPkt = ( (millis() - g_last_radio_packet_ms) <= RX_TIMEOUT_MS ) && !g_failsafe_active;
    for(int i=0;i<8;i++){
      if (hasPkt) g_rc_center[i] = constrain(map(pkt.p[i], 0, 255, PWM_MIN, PWM_MAX), PWM_MIN, PWM_MAX);
      else g_rc_center[i] = (g_rc_center[i] >= PWM_MIN && g_rc_center[i] <= PWM_MAX) ? g_rc_center[i] : 1500;
    }
    saveRcCalibrationToNVS();
 }
 ...
 void taskRadioComm(void *pvParameters) {
   ...
   for (;;) {
     ...
     if (radio.available()) {
       ...
       if (pkt.s1 == 1) g_manual_mode = true; else g_manual_mode = false;
     }
       ...
       if (!error) {
         ...
         if (strcmp(cmd, "get") == 0) {
            ...
            if (strcmp(param, "pids") == 0) {
              ...
              if (xSemaphoreTake(g_serial_mutex, portMAX_DELAY)) {
                StaticJsonDocument<512> res; res["status"] = "data";
                ...
                for (int i=0;i<8;i++){ centers.add(g_rc_center[i]); trimArr.add(g_rc_trim[i]); }
            ...
            if (strncmp(p, "kp_", 3) == 0 || strncmp(p, "ki_", 3) == 0 || strncmp(p, "kd_", 3) == 0){
              ...
              StaticJsonDocument<32> res; res["status"]="ok"; sendJsonSerial(res);
            }
            // Trims
            ...
            else if (strcmp(p, "trim_servo_l") == 0) { g_servo_trim_l = (int)v; saveRcCalibrationToNVS(); }
            else if (strcmp(p, "trim_servo_r") == 0) { g_servo_trim_r = (int)v; saveRcCalibrationToNVS(); }
         } 
         else if (strcmp(cmd, "action") == 0) {
            if(strcmp(doc["name"], "cal_gyro")==0) { g_request_gyro_cal = true; }
            else if(strcmp(doc["name"], "save_pids")==0) savePidsToNVS();
            else if(strcmp(doc["name"], "cal_rc")==0) { g_request_rc_cal = true; }
            else if(strcmp(doc["name"], "center_servos")==0) { g_request_center_servos = true; }
            else if(strcmp(doc["name"], "center_and_save")==0) { g_request_center_and_save = true; }
         }
       }
     }
 ...
 void loop() {
   ...
   if (g_request_rc_cal) { performRcCalibration(5000); g_request_rc_cal = false; }
   if (g_request_center_servos) { performCenterServos(2000); g_request_center_servos = false; }
   if (g_request_center_and_save) { performCenterAndSave(); g_request_center_and_save = false; }
   ...
   g_telemetry_roll = g_input_roll; g_telemetry_pitch = g_input_pitch;
