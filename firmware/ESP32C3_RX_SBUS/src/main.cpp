...
// Plataforma: ESP32-C3 Mini (RISC-V, single-core, USB CDC nativo)
// Parte do OpenRC-AeroLink — receptor hobby.
// =============================================================================
// HISTÓRICO:
//   v1 - SBUS via Arduino HardwareSerial Serial1, invert=true (não decodificou
//        na Matek MOBULA6/MATEKF411RX, mesmo frame válido e polaridade OK)
//   v2 - SBUS via IDF UART_NUM_1 + uart_set_line_inverse (idem)
//   v3 - SBUS via IDF UART_NUM_0 (idem)
//   v4 - SBUS via RMT bit-bang (idem)
//   v5 - PPM single-wire via RMT  ← FUNCIONOU! Solução adotada.
//
...
//
// PINAGEM (ESP32-C3)
//   ┌──── nRF24L01 PA+LNA ────┐         ┌──── FC (MATEKF411RX/MOBULA6) ───┐
//   │  GND   ── GND           │         │  GND  ── GND                     │
//   │  VCC   ── 3V3           │         │  PPM  ── GPIO21 (RMT)            │
//   │  CE    ── GPIO10        │         │  5V   ── (5V só com bateria)     │
//   │  CSN   ── GPIO7         │         └─────────────────────────────────┘
//   │  SCK   ── GPIO4         │
//   │  MOSI  ── GPIO6         │     Serial debug: USB CDC nativo (Serial)
//   │  MISO  ── GPIO5         │
//   │  IRQ   ── (não conectado)
//   └─────────────────────────┘
//
// ⚠️ HARDWARE
//   - Capacitor 10–100 µF entre VCC e GND do nRF24 (resets sob TX).
//   - GND ESP32-C3 ↔ GND FC OBRIGATÓRIO.
//   - NUNCA ligar USB do C3 E bateria simultaneamente (current fight).
// =============================================================================
...
static constexpr uint16_t PPM_PULSE_HIGH_US = 300;
static constexpr uint16_t PPM_CHANNEL_MIN_US = 1000;
static constexpr uint16_t PPM_CHANNEL_MID_US = 1500;
static constexpr uint16_t PPM_CHANNEL_MAX_US = 2000;
static constexpr uint16_t PPM_SYNC_GAP_US   = 8000;
...
// 0..255 (byte do TX V2) → 1000..2000us (PPM). Center 128→~1500us.
...
void loop() {
  ...
  // Manda frame PPM a cada ~20 ms (50 Hz)
  uint32_t now = micros();
